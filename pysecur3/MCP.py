...
class MCPPacket:
	...
	def __repr__(self):
		t = ''
		t +=  '= NetworkDatagram =\r\n'
		t += f'SRC_MAC        : {self.SRC_MAC.hex().upper()}\r\n' 
		t += f'DST_MAC        : {self.DST_MAC.hex().upper()}\r\n' 
		t += f'checksum       : {hex(self.checksum).upper() if self.checksum else ""}\r\n' 
		t += repr(self.payload)
		return t		
...
class MCP():
	...
	def __repr__(self):
		t = ''
		t += '----------\r\nMCP:\r\n'
		t += f'      isResponse: {self.isResponse}\r\n'
		t += f'      Length    : {self.length}\r\n'
		t += f'      Tag       : {hex(self.tag).upper()}\r\n'
		t += f'      Token     : {hex(self.token).upper()}\r\n'
		t += f'      Command ID: {self.command_id} ({MCPCommand(self.command_id).name})\r\n' 
		t += f'      Checksum  : {hex(self.checksum).upper() if self.checksum else ""}\r\n' 
		t += repr(self.command)	 # insert string repr from the command itself
...
class MCPGenericCommand:
	...
	def to_bytes(self, length = 1):		
		if isinstance(self.value, str):
			self.value = (int(self.value, 16))				
		length = round((len(hex(self.value)) -2)/2) 
		val_bytes = self.value.to_bytes(length, byteorder = 'big', signed = False)
...
class MCPGetTransition:
	def __init__(self):
		self.port_id = None
	
	@staticmethod
	def construct(port_id = 0):
		t = MCPGetTransition()
		t.port_id = port_id
		return t
		
	def to_bytes(self):
		return self.port_id.to_bytes(1, byteorder = 'big', signed = False)
...
class MCPGetTransitionResponse:
	def __init__(self):
	...
	def from_bytes(data):
		t = MCPGetTransitionResponse()
		t.response = data.hex()
		t.percent_open = int(t.response[0:2], 16)/2 
		return t
		
	def to_bytes(self):
		# t =  self.port_id.to_bytes(1, byteorder = 'big', signed = False)
		t = None
		return t
	...
	def listener(self):
		with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as cs:
			try:
				...
				logging.exception(f'MCPDiscover listening socket exception: {e}')
